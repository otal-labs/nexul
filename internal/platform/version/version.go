...
// Version is stamped at build time, e.g. -ldflags "-X .../version.Version=v0.2.0-beta-003".
...
// v0.2.0-beta-003), else "stable". release.yml applies the same rule when it picks tags.
func Channel() string {
	if Version == "dev" {
		return "dev"
	}
