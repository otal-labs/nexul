...
func (s *Service) CreateWithKind(ctx context.Context, kind, projectID, workspaceID, title, whenToUse, body string, alwaysIncluded bool, via string) (*Memory, error) {
	...
	return nil, fmt.Errorf("%w: kind must be empty or %s; for the interview memory, call memory_create with kind %s, which returns the existing one", apperrs.ErrInvalid, KindDecisionsLog, KindInterview)
}
