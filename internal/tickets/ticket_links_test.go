...
func TestMCPTools_TicketLinks(t *testing.T) {
	repo := newFakeRepo()
	s := newTestService(repo)
	ts := seedTickets(t, s, "bug", "origin")
	tools := MCPTools(s)
	call := func(name string, args map[string]any) (any, error) {
		return toolByName(t, tools, name).Call(t.Context(), args)
	}

	_, err := call("ticket_add_blocker", map[string]any{"id": ts[0].ID})
	require.ErrorIs(t, err, apperrs.ErrInvalid)
	_, err = call("ticket_set_found_in", map[string]any{})
	require.ErrorIs(t, err, apperrs.ErrInvalid)
	for _, name := range []string{"ticket_get_ticket_links", "ticket_remove_found_in"} {
		_, err = call(name, map[string]any{})
		require.ErrorIs(t, err, apperrs.ErrInvalid, name)
	}
	_, err = call("ticket_remove_blocker", map[string]any{"id": ts[0].ID})
	require.ErrorIs(t, err, apperrs.ErrInvalid)

	got, err := call("ticket_add_blocker", map[string]any{"id": ts[0].ID, "blocker_id": ts[1].ID})
	require.NoError(t, err)
	assert.True(t, got.(*LinkSet).Blocked)
	blocked, err := call("ticket_list_blocked", map[string]any{})
	require.NoError(t, err)
	assert.Len(t, blocked.(map[string][]LinkedTicket)[ts[0].ID], 1)
	got, err = call("ticket_remove_blocker", map[string]any{"id": ts[0].ID, "blocker_id": ts[1].ID})
	require.NoError(t, err)
	assert.Empty(t, got.(*LinkSet).BlockedBy)

	got, err = call("ticket_set_found_in", map[string]any{"id": ts[0].ID, "origin_unknown": true})
	require.NoError(t, err)
	assert.True(t, got.(*LinkSet).OriginUnknown)
	got, err = call("ticket_set_found_in", map[string]any{"id": ts[0].ID, "origin_id": ts[1].ID})
	require.NoError(t, err)
	assert.Equal(t, ts[1].ID, got.(*LinkSet).FoundIn.ID)
	got, err = call("ticket_get_ticket_links", map[string]any{"id": ts[1].ID})
	require.NoError(t, err)
	assert.Len(t, got.(*LinkSet).BugsFound, 1)
	got, err = call("ticket_remove_found_in", map[string]any{"id": ts[0].ID})
	require.NoError(t, err)
	assert.Nil(t, got.(*LinkSet).FoundIn)
}
