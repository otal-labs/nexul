...
describe("WizardBranchesStep", () => {
  it("seeds the default branch's row with the service's hostname and network", async () => {
    renderStep();
  ...
  it("saves a wildcard row on its own network as a rule next to the default branch's", async () => {
    ...
    await pickOption(user, "Network", "qa_default — postgres, redis");
    expect(screen.queryByText(productionWarning)).not.toBeInTheDocument();
    ...
    expect(mocks.patch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        branch_deploy_rules: [
          ...
          { pattern: "feature/*", docker_network: "qa_default", hostname_template: "{branch}.example.com", port: 80 },
        ],
      }),
    );
